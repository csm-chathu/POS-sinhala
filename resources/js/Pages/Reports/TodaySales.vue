    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-bold" style="color:#0F172A;">{{ t('rep.today') }} — {{ date }}</h1>
            </div>
        </template>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white rounded-xl p-4 shadow-sm" style="border:1px solid #E2E8F0;">
                <p class="text-xs text-slate-500 mb-1">{{ t('th.method') }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm" style="border:1px solid #E2E8F0;">
                <div class="px-4 py-3" style="border-bottom:1px solid #E2E8F0;">
                    <h2 class="font-semibold" style="color:#0F172A;">{{ t('rep.by_method') }}</h2>
                </div>
                <div class="p-4 space-y-3">
                    <div v-if="byPaymentMethod.length === 0" class="text-center text-slate-400 py-6">{{ t('rep.no_sales') }}</div>
                    <div v-for="pm in byPaymentMethod" :key="pm.method" class="flex justify-between items-center py-2" style="border-bottom:1px solid #F1F5F9;">
                        <div>
                            <p class="font-medium" style="color:#0F172A;">{{ t('lbl.' + pm.method) || pm.method }}</p>
                            <p class="text-xs text-slate-500">{{ pm.count }}</p>
                        </div>
            ...
            <div class="lg:col-span-2 bg-white rounded-xl shadow-sm" style="border:1px solid #E2E8F0;">
                <div class="px-4 py-3" style="border-bottom:1px solid #E2E8F0;">
                    <h2 class="font-semibold" style="color:#0F172A;">{{ t('nav.sales') }}</h2>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead>
                            <tr class="text-xs text-slate-500 uppercase" style="background:#F8FAFC; border-bottom:1px solid #E2E8F0;">
                                <th class="px-4 py-3 text-left">{{ t('th.invoice') }}</th>
                                <th class="px-4 py-3 text-left">{{ t('th.cashier') }}</th>
                                <th class="px-4 py-3 text-right">{{ t('th.total') }}</th>
                                <th class="px-4 py-3 text-right">{{ t('th.time') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-if="sales.length === 0"><td colspan="4" class="px-4 py-8 text-center text-slate-400">{{ t('rep.no_sales') }}</td></tr>
